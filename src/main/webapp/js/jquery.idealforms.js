...
;(function ( $, window, document, undefined ) {
  ...
  var Utils = {
    ...
    getByNameOrId: function( str ) {
      var $el = $('[data-rule="'+ str +'"]').length
          ? $('[data-rule="'+ str +'"]') // by name
          : $('#'+ str) // by id
      return $el.length
          ? $el
  ...
  $.fn.idealTabs = function (container) {
    ...
    $contents.each(function () {
      var $this = $(this), name = $(this).attr('data-rule')
      $this.data('ideal-tabs-content-name', name)
          .removeAttr('data-rule')
    })
  ...
  $.fn.idealSelect = function () {
    ...
    return this.each(function () {
      ...
      var idealSelect = (function () {
        var
            $wrap = $('<ul class="ideal-select '+ $select.attr('data-rule') +'"/>'),
            $menu = $(
  ...
  $.idealforms.errors = {
    ...
    number: '必须是数字.',
    digits: '必须是数字.',
    name: '必须至少3个字符长,必须只包含字母.',
    username: '只能是中文，长度为2-7位',
    pass: '必须由数字、字母或特殊字符组成,密码长度不能少于8个字符.',
    strongpass: '必须至少8个字符长,包含至少一个大写字母和一个小写字母和一个数字或特殊字符.',
    email: '必须是一个有效的电子邮件地址. <em>(例如: user@95081.com)</em>',
    phone: '必须是一个有效的电话号码. <em>(例如: 1881234567)</em>',
    tel:'必须是一个有效的座机号码. <em>(例如: 84526891 或者 010-84526891)</em>',
    zip: '必须是一个有效的邮政编码. <em>(例如: 100040)</em>',
    url: '必须是一个有效的URL. <em>(例如: www.google.com)</em>',
    minChar: '必须至少 <strong>{0}</strong> 字符长.',
    minOption: '检查至少 <strong>{0}</strong> 选项.',
    maxChar: '不超过 <strong>{0}</strong> 字符长.',
    maxOption: '不超过 <strong>{0}</strong> 允许选项.',
    range: '必须是 {0} 到 {1}之间的数字.',
    date: '必须是一个有效的日期. <em>(例如: {0})</em>',
    dob: '必须是一个有效的出生日期.',
    exclude: '"{0}" 是不可用的.',
    excludeOption: '{0}',
    equalto: '必须是相同的值 <strong>"{0}"</strong>',
    extension: '文件必须有一个有效的扩展. <em>(例如: "{0}")</em>',
    ajaxSuccess: '<strong>{0}</strong> 是不可用的.',
    ajaxError: '服务器错误...',
    specialChart:'不得含有特殊字符'
  ...
  var getFilters = function() {
    ...
    var filters = {
      ...
      phone: {
        regex: /^(13[0-9]|14[57]|15[0-9]|17[0-9]|18[0-9])\d{8}$/,
        error: $.idealforms.errors.phone
      },
      tel: {
    	  regex: /(\(\d{3,4}\)|\d{3,4}-|\s)?\d{8}/,
    	  error: $.idealforms.errors.tel
      },
      specialChart: {
    	  regex: /[`~!@#$^&*()=|{}':;',\\[\\].<>\/?~！@#￥……&*（）——|{}【】‘；：”“'。，、？?]/,
    	  error: $.idealforms.errors.specialChart
      },
      ...
      equalto: {
        regex: function( input, value ) {
          var $equals = $( input.userOptions.data.equalto ),
              $input = input.input,
              name = $equals.attr('data-rule') || $equals.attr('id'),
      ...
      ajax: {
        regex: function( input, value, showOrHideError ) {
          ...
          // Send input name as $_POST[data-rule]
          var data = {}
	  ...
	  if(options.inputs!= undefined){
		  ...
		  options.inputs =  {
	            ...
	            'langs[]': {
	                ...
	                data: { min: 2, max: 3 },
	                errors: {
	                    min: '至少选择 <strong>2</strong> 个.',
	                    max: '最多不超过 <strong>3</strong> 个.'
	                }
	            }
	        }
		  return  this.each(function() {
...
// Get LESS variables
  ...
  $.extend( IdealForms.prototype, {
    ...
    _init: function() {
      ...
      // Generate tabs
      if ( self.$tabs.length ) {
        var $tabContainer = $('<div class="ideal-wrap ideal-tabs"/>')
    ...
    _getUserInputs: function() {
      return this.$form.find('[data-rule="'+ Utils.getKeys( this.opts.inputs ).join('"], [data-rule="') +'"]')
    },
    ...
    _adjust: function() {
      ...
      labels.removeAttr('style').width(90+"px" )
    ...
    _validate: function( $input, e ) {
      ...
      var userOptions = o.inputs[ $input.attr('data-rule') ]
      ...
      var inputData = {
        ...
        input: isRadioCheck ? self.$form.find('[data-rule="' + name + '"]') : $input,
    ...
    _responsive: function() {
      ...
      var maxWidth = formElements.labels.outerWidth()
  ...
  $.extend( IdealForms.prototype, {
    ...
    toggleFields: function( fields ) {
      ...
      $fields.each(function() {
        var $this = $(this)
        var name = $this.attr('data-rule') || $this.attr('id')
